...
export default function Women() {
  ...
  useEffect(() => { fetchWomenGarments(); }, []);

  const fetchWomenGarments = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/garments');
      const data = await response.json();
      setGarments(data.filter((g) => g.category?.trim().toLowerCase() === 'female'));
    } catch (error) {
      console.error('Error fetching women garments:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const applyFilters = (list) => {
    let result = subCategory
      ? list.filter((g) => g.subCategory?.toLowerCase() === subCategory.toLowerCase())
      : list;
    if (search.trim()) {
    ...
    if (filters.subcategories.length > 0) {
      result = result.filter((g) => filters.subcategories.includes(g.subCategory));
    }
    if (filters.fabrics.length > 0) {
      result = result.filter((g) => filters.fabrics.includes(g.fabric));
    }
    ...
    if (filters.sizes.length > 0) {
      result = result.filter((g) => (g.sizes || []).some((s) => filters.sizes.includes(s)));
    }
    return result;
  };

  const filteredGarments = applyFilters(garments);

  return (
    <div className="home-container">
      <header className="newarrival-hero">
        <div className="newarrival-hero-content">
          <h1>{subCategory ? `${subCategory} Collection` : 'Women Collection'}</h1>
        </div>
      </header>
        ...
        <div className="shop-products">
          <div className="shop-results-bar">
            <span className="shop-results-count">
              {filteredGarments.length} item{filteredGarments.length !== 1 ? 's' : ''} found
            </span>
          </div>

          {loading ? (
            <div className="loading-state">Loading Women's Collection...</div>
                      ...
                      <div className="garment-sizes">
                        {garment.sizes.map((size, idx) => (
                          <button
                            ...
                            onClick={() => setSelectedSizes((prev) => ({ ...prev, [garment._id]: size }))}
                          >
                            {size}
                          </button>
                        ))}
