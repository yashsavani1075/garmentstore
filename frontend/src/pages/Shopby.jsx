...
export default function Shopby() {
  ...
  const filteredGarments = (() => {
    ...
    if (filters.subcategories.length) r = r.filter((g) => filters.subcategories.includes(g.subCategory));
    if (filters.fabrics.length) r = r.filter((g) => filters.fabrics.includes(g.fabric));
    ...
    if (filters.sizes.length) r = r.filter((g) => (g.sizes || []).some((s) => filters.sizes.includes(s)));
    return r;
  })();
