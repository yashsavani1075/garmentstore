...
export default function Men() {
  ...
  useEffect(() => { fetchMenGarments(); }, []);

  const fetchMenGarments = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/garments`);
      const data = await response.json();
      setGarments(data.filter((g) => g.category?.trim().toLowerCase() === 'male'));
    } catch (error) {
      console.error('Error fetching men garments:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="home-container">
      <header className="newarrival-hero">
        <div className="newarrival-hero-content">
          <h1>{subCategory ? `${subCategory} Collection` : 'Men Collection'}</h1>
        </div>
      </header>
          ...
          {loading ? (
            ...
            <div className="garments-grid">
              {filteredGarments.map((garment) => {
                ...
                return (<div key={garment._id} className="garment-card">
                  <div
